                  ...
                  authenticateUser(user)
                    .map(
                      _.withCookies(Cookie(name, value, httpOnly = false))
                        ...
                        .withCookies(Cookie("city", URLEncoder.encode(user.city, "UTF-8"), httpOnly = false))
                        .withCookies(Cookie("address", URLEncoder.encode(user.address, "UTF-8"), httpOnly = false))
                    )
                case None =>
                  Future
